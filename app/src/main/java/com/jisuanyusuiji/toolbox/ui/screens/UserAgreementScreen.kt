...
一、服务说明
1. 本软件是一款运行于用户设备本地（Android 平台）的工具箱，提供随机工具、计算与转换、文本与编码、图片与视频处理、学习查询等功能。
2. 本软件默认完全离线运行；联网为可选功能（背单词在线补充释义/例句、检查版本更新），用户可在“设置 → 联网功能”中随时关闭。除此之外不收集、不上传用户的任何数据、文件或个人信息。
...
三、数据与隐私
1. 本软件所有配置、历史、模板、背单词分区等数据均保存在用户设备本地。
2. 卸载本软件或清除应用数据将导致本地数据丢失，请自行做好备份。
3. 本软件仅在用户开启联网功能时访问网络，用于在线词典与版本检查，不会上传用户的任何文件或个人信息；可随时在设置中关闭。使用手电筒、镜子时需申请相机权限，使用录音机时需申请麦克风权限，相关功能均在本机完成，不会上传或分享。
4. 用户导入的词库、录音、备忘录等数据仅保存在本机应用目录中，卸载后会被删除。
